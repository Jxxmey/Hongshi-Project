...
export default function ArtistProfile() {
  // อัปเดตข้อมูล Tags ตาม Behind the Stage ล่าสุด
  const hoshiTags = [
    "กินไอศกรีม 2 แกลลอนคนเดียว! 🍨",
    "พกช้อนส่วนตัว 🥄",
    "Pop Culture Geek (Kaiju No. 8) 👾",
    "อิน Inazuma Eleven ⚽",
    "เป็นคนขี้กังวล (Worrywart) 🥺",
    "ไม่ใช่คนตื่นเช้า 🛌",
    "ชอบกลิ่นอายสนามบิน ✈️",
    "เกลียดเสียงเล็บขูดกระดาษ 💅📄",
    "อยากเลี้ยงตุ่นปากเป็ด 🦆",
    "ไอดอลคือ 'IU' 🎵",
    "สาย K-indie / K-R&B 🎧",
    "รักการถ่ายภาพฟิล์ม 📸"
  ];
  ...
  return (
    <div className="py-12 px-4 max-w-5xl mx-auto space-y-20 selection:bg-azalea selection:text-white pb-20">
      ...
      <section className="text-center space-y-6">
        <h1 className="text-5xl md:text-6xl font-heading font-bold text-navy">HONGSHI</h1>
      ...
      <section className="grid md:grid-cols-2 gap-8 items-center bg-white p-8 md:p-10 rounded-3xl shadow-sm border-t-8 border-skyblue">
        <div className="space-y-4 font-body text-navy">
          <h3 className="text-3xl font-heading font-bold mb-6 text-navy">📝 Profile</h3>
          <p><strong>ชื่อ-นามสกุล:</strong> พิเชฐพงศ์ จิรเดชสกุลวงศ์</p>
          <p><strong>AKA:</strong> Hong (ฮง), HONGSHI (ฮงชิ), Hong LYKN</p>
          <p><strong>วันเกิด:</strong> 16 ตุลาคม 2546</p>
          <p><strong>ส่วนสูง / น้ำหนัก:</strong> 178 ซม. / 68 กก. (กรุ๊ปเลือด O)</p>
          <p><strong>การศึกษา:</strong> โรงเรียนสวนกุหลาบวิทยาลัย, สถาบันนวัตกรรมบูรณาการ จุฬาฯ (BAScii)</p>

          <div className="pt-4 border-t border-gray-100 flex flex-wrap gap-3">
            <a href="https://www.instagram.com/hongshihoshi" target="_blank" rel="noreferrer" className="bg-skyblue text-navy px-5 py-2 text-sm font-bold rounded-full hover:bg-azalea transition-colors shadow-sm">IG: @hongshihoshi</a>
            <a href="https://twitter.com/hongshihoshi03" target="_blank" rel="noreferrer" className="bg-skyblue text-navy px-5 py-2 text-sm font-bold rounded-full hover:bg-azalea transition-colors shadow-sm">X: @hongshihoshi03</a>
            <a href="https://www.tiktok.com/@hongshihoshi" target="_blank" rel="noreferrer" className="bg-skyblue text-navy px-5 py-2 text-sm font-bold rounded-full hover:bg-azalea transition-colors shadow-sm">TikTok: @hongshihoshi</a>
            <a href="https://www.instagram.com/introduction2youth" target="_blank" rel="noreferrer" className="bg-skyblue text-navy px-5 py-2 text-sm font-bold rounded-full hover:bg-azalea transition-colors shadow-sm">📸 Photo Gallery</a>
          </div>
        </div>
        <div className="rounded-2xl overflow-hidden shadow-inner border-4 border-palepink relative aspect-square md:aspect-auto h-full min-h-[300px]">
      ...
      <section className="space-y-8">
        ...
        <div className="border-l-4 border-skyblue ml-4 md:ml-[50%] space-y-10 py-4 font-body">
          {[
            { date: "วัยมัธยม (อายุ 14 ปี)", title: "จุดเริ่มต้นจากสายเต้น", desc: "เริ่มต้นฝึกฝนการเต้นฮิปฮอปอย่างจริงจัง เป็นเชียร์ลีดเดอร์ นักเต้นโรงเรียน และฟอร์มวงโคฟเวอร์แดนซ์ K-Pop ชื่อ 'TNT'" },
      ...
      {/* --- New Section: Series Spotlight Twenty One --- */}
      ...
      <section className="text-center space-y-8 pb-10">
        <div className="space-y-2">
          <h2 className="text-3xl font-heading font-bold text-navy">🍦 The Real HONGSHI</h2>
          <p className="font-body text-navy/80">มุมน่ารักๆ ที่ทำให้แฟนคลับ (LYKYOU) ตกหลุมรัก</p>
        </div>
        <div className="flex flex-wrap justify-center gap-3 font-body text-sm md:text-base max-w-4xl mx-auto">
          {hoshiTags.map((tag, index) => (
            <span key={index} className="bg-skyblue text-navy font-bold px-5 py-3 rounded-full shadow-sm hover:bg-azalea transition-colors duration-300 cursor-default">
              {tag}
            </span>
          ))}
        </div>
